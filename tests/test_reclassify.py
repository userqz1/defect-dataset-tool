...
def _combo(pane: AnnotationPane, row: int):
    ...
    return widget.findChild(EditableComboBox)
...
def test_class_outside_the_project_list_is_still_shown(qapp):
    """Real data has these — e.g. a "TODO" placeholder from an import.

    Opening the dropdown must not silently retarget such a shape to
    whatever happened to be first.
    """
    pane = _pane_with_rows(["TODO"])
    combo = _combo(pane, 0)
    assert combo.currentText() == "TODO"
    assert "TODO" in [combo.itemText(i) for i in range(combo.count())]


def test_rebuilding_the_list_emits_nothing(qapp):
...
def test_picking_a_class_asks_for_the_reclassify(qapp):
    ...
    pane._on_row_class_committed(1, combo)
    assert seen == [(1, "unmarked_normal")]


def test_committing_the_unchanged_class_is_a_noop(qapp):
    pane = _pane_with_rows(["marked_loose"])
    seen = []
    pane.rename_shape_requested.connect(lambda r, c: seen.append((r, c)))
    pane._on_row_class_committed(0, _combo(pane, 0))
    assert seen == []
