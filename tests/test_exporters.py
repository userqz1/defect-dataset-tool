...
def _write_image(path: Path, color: tuple[int, int, int] = (200, 200, 100)) -> None:
    """Write a 64x64 solid-color PNG for the exporter to read."""
    im = Image.new("RGB", (64, 64), color)
    im.save(path)


def _write_labelme(json_path: Path, image_path: Path, label: str) -> None:
    """Write a minimal LabelMe JSON with a single rectangle shape."""
    payload = {
        "version": "5.0.1",
        "shapes": [
            {
...
def synthetic_dataset(tmp_path: Path) -> Dataset:
    """Create a real-on-disk 6-image, 2-category, all-labeled dataset."""
    root = tmp_path / "raw"
    cats: list[Category] = []

    # 4 images of class "cat", 2 of class "dog"
