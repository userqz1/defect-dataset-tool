"""Export to Pascal VOC format — thin wrapper delegating to ``format_out``."""
...
def export_voc(
    ...
    result = export_samples(ss, "voc", fo, progress_cb=progress_cb)

    return ExportReport(
        written_images=result.written_images,
        written_xml=result.written_labels,
