"""Export to YOLO format — thin wrapper delegating to ``format_out``.

Preserves the ``export_yolo(SplitResult, opts)`` API used by the
schema / pipeline subsystem, but internally converts to ``SampleSet``
and calls the unified writer.  No more ``parse_annotation()`` calls.
"""
...
def export_yolo(
    ...
    result = export_samples(ss, "yolo", fo, progress_cb=progress_cb)
