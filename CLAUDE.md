...
- **GUI code is testable offscreen — including real widgets.** The recipe is `os.environ.setdefault("QT_QPA_PLATFORM", "offscreen")` + `pytest.importorskip("PyQt6", …)` before the `gui` import, so files skip cleanly where PyQt is absent. Two flavours exist: pure functions (`tests/test_shape_conventions.py` exercises the `_region_to_shape` / `_shape_to_region` bridge — dataclasses in, dataclasses out) and live widgets (`tests/test_viewer_key_bubbling.py`, `tests/test_viewer_hud_layout.py` instantiate `ImageViewer`, send real `QKeyEvent`s and measure geometry).
- **Widget tests MUST use the session-scoped `qapp` fixture from `conftest.py`** — never build a `QApplication` per module. If one is garbage-collected between modules it takes qfluentwidgets' global `qconfig` singleton with it, and every widget built afterwards dies with `RuntimeError: wrapped C/C++ object of type QConfig has been deleted`. The failure only appears when two GUI modules run in the same session, so it passes file-by-file and breaks the full suite. `conftest.py` holds the app in a module global specifically to prevent this.
