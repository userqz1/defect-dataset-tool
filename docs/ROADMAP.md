...
预标注作为阶段 1 和阶段 2 之间的过渡能力，不直接替代人工标注，也不直接等同于训练结果。它的目标是减少重复画框工作，并把模型输出变成可审核的数据资产。

第一版只做 YOLO 检测预标注：

- 输入：用户选择本地 YOLO 权重、置信度阈值和图片范围。
- 输出：矩形框、类别、置信度，并写入项目主标注层。
- 状态：模型输出进入 `prelabeled`，人工确认后进入 `ready`。
- 范围：当前图、当前类目、未标注图片、全部图片。
- 保护：默认跳过已有人工标注，覆盖必须显式确认。

第一版不做：

- 不内置下载大模型或自动训练。
- 不把默认 COCO 权重当作工业缺陷模型。
- 不直接接 ChatGPT Pro 会话或非官方反代。
- 不把预标注结果直接视为可交付真值。

后续扩展顺序：

1. YOLO 本地权重预标注。
2. SAM / SAM2 基于框或点的分割辅助。
3. GroundingDINO / YOLO-World 文本提示检测。
4. OpenAI API / VLM 只处理少量疑难样本和图文说明。
